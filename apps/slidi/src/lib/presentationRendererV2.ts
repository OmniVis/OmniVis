...
function Presentation() {
    ...
    if (content.visualType === "icon" && content.visualPrompt) {
      return (
        <div className="sl-anim-scale sl-interactive-card" style={{ 
          ...
          padding: 'clamp(30px, 4cqw, 60px)', borderRadius: '32px',
  ...
  // ── Typography scale ─────────────────────────────────────────────────
  const eyebrowStyle = {
    fontSize:'clamp(14px,1.4cqw,22px)', fontWeight:800, textTransform:'uppercase',
    letterSpacing:'0.2em', color:'var(--sl-accent)',
    display:'block', flexShrink:0, overflow:'hidden', whiteSpace:'nowrap', textOverflow:'ellipsis'
  };
  const h1Style = {
    fontSize:'clamp(64px,6.5cqw,130px)', fontWeight:900, lineHeight:1.05,
    letterSpacing:'-0.03em', margin:0, flexShrink:0, color: 'var(--sl-text)',
    overflow:'hidden', whiteSpace:'pre-wrap', wordBreak:'break-word'
  };
  const h2Style = {
    fontSize:'clamp(44px,4.8cqw,88px)', fontWeight:800, lineHeight:1.1,
    letterSpacing:'-0.02em', margin:0, flexShrink:0, color: 'var(--sl-text)',
    overflow:'hidden', wordBreak:'break-word',
    display:'-webkit-box', WebkitLineClamp:4, WebkitBoxOrient:'vertical'
  };
  const h3Style = {
    fontSize:'clamp(26px,2.8cqw,46px)', fontWeight:700, lineHeight:1.2,
    letterSpacing:'-0.01em', color: 'var(--sl-text)',
    margin:0, overflow:'hidden', wordBreak:'break-word', flexShrink:0
  };
  const bodyStyle = {
    fontSize:'clamp(22px,2.2cqw,36px)', lineHeight:1.6, color:'var(--sl-sub)',
    fontWeight: 400,
    margin:0, overflow:'hidden', wordBreak:'break-word',
    display:'-webkit-box', WebkitLineClamp:5, WebkitBoxOrient:'vertical'
  };
  const labelStyle = {
    fontSize:'clamp(16px,1.6cqw,22px)', fontWeight:700, textTransform:'uppercase',
    letterSpacing:'0.1em', color:'var(--sl-accent)', margin:0, flexShrink:0
  };

  // ── Layout constants ──────────────────────────────────────────────────
  const padV = 'clamp(48px,7cqh,100px)';
  const padH = 'clamp(64px,8cqw,140px)';
  const gap   = 'clamp(24px,3.5cqh,48px)';
  const gapSm = 'clamp(14px,1.8cqh,28px)';
