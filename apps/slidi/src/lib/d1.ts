/**
 * PostgreSQL database client — wraps pg.Pool with the same
 * d1Query / d1First / d1Run interface previously used by the Cloudflare
 * Worker proxy. All callers are unchanged.
 *
 * SQLite-style ? placeholders are automatically converted to PostgreSQL's
 * $1 / $2 / ... positional parameters, so no API route needs updating.
 */
