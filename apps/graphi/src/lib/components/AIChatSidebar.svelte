...
<div
    ...
    {#each messages as msg (msg.timestamp)}
      <div class="flex items-start gap-3">
        ...
        <div class="min-w-0 flex-1">
          {#if msg.role === 'user'}
            <div
              class="max-w-[85%] border border-border bg-muted px-4 py-2.5 text-xs leading-relaxed break-words text-foreground">
              {msg.content}
            </div>
          {:else}
            <div class="flex w-[290px] flex-col gap-2">
              {#each parseMessageBlocks(msg.content) as block, idx (idx)}
                {#if block.type === 'text'}
                  <div
                    class="border border-border bg-muted/30 p-3 text-xs leading-relaxed break-words {idx ===
                    0
                      ? 'border-t-0'
                      : ''}">
                    {block.content}
  ...
  <div class="absolute right-0 bottom-0 left-0 z-30 border-t border-border/50 bg-surface px-4 py-4">
    <div
      class="border border-border bg-background p-3 transition-all duration-300 focus-within:border-primary/50">
      <textarea
